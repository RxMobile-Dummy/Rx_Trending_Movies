...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
  ...
  func scene(_ scene: UIScene, willConnectTo session: UISceneSession, options connectionOptions: UIScene.ConnectionOptions) {
    ...
    UIApplication.shared.statusBarUIView?.backgroundColor = Config.BTN_THEME_COLOR
    let userId = kUSERDEFAULT.object(forKey: kCurrentUser) as? String
    if ((userId ?? "") != "") {
      Utility.navigateToTabBarVC(window: self.window!, isPush: true)
    } else {
      Utility.navigateToLoginVC(window: self.window!, isPush: true)
    }
  }
  ...
  func setStatuBarColor() -> UIView {
    if #available(iOS 13.0, *) {
      ...
      if responds(to: Selector(("statusBar"))) {
        return value(forKey: "statusBar") as? UIView ?? UIView()
      }
    }
    return UIView()
  }
