...
class SignUpView: NSObject {
  ...
  func checkValidationForLogin(vc : SignUpVC) -> Bool {
    ...
    let email = SignUpView.validation.checkEmptyAndValidEmailValidation(email: vc.createUser.email)
    let password = SignUpView.validation.checkEmptyAndValidPasswordValidation(password: vc.createUser.password)
